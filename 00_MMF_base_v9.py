"""MMF_base_v9: Add summary dataframe containing quantity of snacks ordered and
all profit calculations. Also made small changes to the program
Jack Andrews
18/03/22
"""
...
def loop(movie_data_dict):
    constants = namedtuple("consts", ["MAX_TICKETS", "SURCHARGE_RATE",
                                      "SNACK_PROFIT_MARGIN"])
    consts = constants(5, .05, .2)  # Creating the MAX_TICKETS constant
...
def main():
    import pandas as pd
    constants = namedtuple("consts", ["MAX_TICKETS", "SNACK_PROFIT_MARGIN"])
    consts = constants(5, .2)
    all_names = []
    all_tickets = []
    surcharge = []
    snack_cost = []
    ...
    if not snack_cost:  # No snacks ordered
    ...
    summary_data.extend([s_profit := sum(movie_data_dict["Snack Profit"]),
                         ticket_profit, ticket_profit + s_profit])
    # Ticket profit is total profit - snack profit
    summary_frame = pd.DataFrame(summary_data_dict)
    summary_frame = summary_frame.set_index("Item")
    # print(summary_headings)
    print("\n*** Ticket/Snack Information ***\n")
    print("Note: For full details see excel file called {xxx}\n")
